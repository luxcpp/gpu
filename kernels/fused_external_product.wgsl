...
//
// Fused External Product v2 — WGSL compute shaders
// Ported from fused_external_product.metal.
// Zero intermediate global memory writes. Single kernel: decompose+NTT+mul+INTT+acc.
...
// ---------------------------------------------------------------------------
// Workgroup shared: work[N] + tw caches [4*N]
// ---------------------------------------------------------------------------
...
fn ct_butterfly(idx_lo: u32, idx_hi: u32, tw_idx: u32, Q: vec2<u32>) {
    let lo = work[idx_lo];
    let hi = work[idx_hi];
...
fn gs_butterfly(idx_lo: u32, idx_hi: u32, tw_idx: u32, Q: vec2<u32>) {
    let lo = work[idx_lo];
    let hi = work[idx_hi];
    let sum = mod_add(lo, hi, Q);
    let diff = mod_sub(lo, hi, Q);
    work[idx_lo] = sum;
    work[idx_hi] = barrett_mul(diff, s_tw_inv[tw_idx], Q, s_pre_inv[tw_idx]);
}

fn do_fwd_ntt(N: u32, log_N: u32, Q: vec2<u32>, lid: u32) {
    for (var stage = 0u; stage < log_N; stage++) {
        ...
        for (var bf = lid; bf < N / 2u; bf += 256u) {
            let ii = bf / t;
            let jj = bf % t;
            ct_butterfly((ii << (log_N - stage)) + jj, (ii << (log_N - stage)) + jj + t, m + ii, Q);
        }
        workgroupBarrier();
    }
}

fn do_inv_ntt(N: u32, log_N: u32, Q: vec2<u32>, lid: u32) {
    for (var stage = 0u; stage < log_N; stage++) {
        let m = N >> (stage + 1u);
        let t = 1u << stage;
        for (var bf = lid; bf < N / 2u; bf += 256u) {
            let ii = bf / t;
            let jj = bf % t;
            gs_butterfly((ii << (stage + 1u)) + jj, (ii << (stage + 1u)) + jj + t, m + ii, Q);
        }
        workgroupBarrier();
    }
}
...
// ===========================================================================
// Main fused external product kernel (v2)
// Grid: [1, batch_size, 1], Threadgroup: [256, 1, 1]
// ===========================================================================

@compute @workgroup_size(256)
fn fused_external_product_v2(
            ...
            for (var out_c = 0u; out_c < 2u; out_c++) {
                ...
                for (var i = lid; i < N; i += 256u) {
                    ...
                    if (in_c == 0u && l == 0u) {
                        ...
                        result[result_b + out_c * N + i] = mod_add(
                            result[result_b + out_c * N + i], val, Q
                        );
                    }
                }
                workgroupBarrier();

                // Reload for next output component
                if (out_c == 0u) {
                    for (var i = lid; i < N; i += 256u) {
                        let val = rlwe[rlwe_b + in_c * N + i];
                        work[i] = u64_and(u64_shr(val, l * base_log), base_mask);
...
@compute @workgroup_size(256)
fn fused_external_product_accumulate_v2(
    @builtin(workgroup_id)        wgid: vec3<u32>,
    @builtin(local_invocation_id) lid_v: vec3<u32>,
) {
    let batch_idx = wgid.y;
    ...
    for (var in_c = 0u; in_c < 2u; in_c++) {
        for (var l = 0u; l < L; l++) {
            ...
            for (var out_c = 0u; out_c < 2u; out_c++) {
                ...
                for (var i = lid; i < N; i += 256u) {
                    let val = barrett_mul(work[i], N_inv, Q, N_inv_pre);
                    accumulator[acc_b + out_c * N + i] = mod_add(
                        accumulator[acc_b + out_c * N + i], val, Q
                    );
                }
                workgroupBarrier();
...
// Processes BATCHES_PER_TG batches per threadgroup to amortize twiddle prefetch.
// ===========================================================================

@compute @workgroup_size(256)
fn fused_external_product_batch_v2(
    @builtin(workgroup_id)        wgid: vec3<u32>,
    @builtin(local_invocation_id) lid_v: vec3<u32>,
) {
    // Each threadgroup processes one batch element (simplified from Metal 4-per-TG
    // since WGSL does not have per-lane batch indexing in y dim easily)
    let batch_idx = wgid.y;
    let lid = lid_v.x;
    let N = params.N;
    let log_N = params.log_N;
    ...
    // Prefetch twiddles (shared across all batches in this threadgroup)
    for (var i = lid; i < N; i += 256u) {
        s_tw_fwd[i] = fwd_twiddles[i];
        s_pre_fwd[i] = fwd_precon[i];
        s_tw_inv[i] = inv_twiddles[i];
